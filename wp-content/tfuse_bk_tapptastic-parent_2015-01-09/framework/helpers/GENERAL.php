    ...
    function tfuse_file_path($path) {
        ...
        if (is_multisite()) {
            ...
            if (isset($imageParts[1]))
			    ...
			    global $wpdb;
			    $query = $wpdb->prepare("SELECT * FROM $wpdb->posts WHERE guid='%s'", $path);
			    $attachment = $wpdb->get_results($query);
			    if (!is_wp_error($attachment) && !empty($attachment[0]->ID) && $attached_file = get_post_meta($attachment[0]->ID, '_wp_attached_file', true)) {
				    $path = $uploads_dir['basedir'] . '/' . $attached_file;
			    }
		    }
        }
        return $path;
