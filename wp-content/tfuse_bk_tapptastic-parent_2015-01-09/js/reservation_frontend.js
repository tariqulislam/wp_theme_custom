...
function showErrorMessage(textMessage){
    var $=jQuery;
    _message= _form.closest('.contact-send').find('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('error_submiting_form').show();
    _form.hide();
}
...
function showSuccessMessage(textMessage){
    var $=jQuery;
    _message=  _form.closest('.contact-send').find('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('success_submited_form').show();
    _form.hide();
...
function check_fields() {
    ...
    if (_form.find('.'+_theme_index + '_email').length>0) {
        ...
        {
            _form.find('.notice').hide(); _form.find('#send_reservation').val('Sending...');
            _form.closest('.contact-send').find('#header_message').hide();
