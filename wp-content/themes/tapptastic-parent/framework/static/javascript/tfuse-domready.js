...
function tf_optionize() {
    ...
    /**** Multi options ****/
    function tfuse_initialize_multi( that ) {
        if( this.counter == undefined ) {
            this.counter = 0;
        }
        if($(that).prop('initialized')!==true) {
            $(that).prop('initialized',true);

            $(that).attr('id', $(that).attr('id') + '-' + this.counter);
            this.counter++;
    ...
    $('.tfuse_suggest_input').each(function(){
        tfuse_initialize_multi( this );
    });

    $(document).bind('DOMNodeInserted',function( e ){
        var object = jQuery(e.target);
        if( object.find('.tfuse_suggest_input').length > 0 ) {
            object.find('.tfuse_suggest_input').each(function(){
                tfuse_initialize_multi( this );
            });
        }
    });
