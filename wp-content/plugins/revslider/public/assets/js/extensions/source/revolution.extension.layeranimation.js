...
jQuery.extend(true,_R, {
			...
			if (startSlideAnimAt!==undefined) {		
				...
				if (opt.timelines.staticlayers) 				
					jQuery.each(opt.timelines.staticlayers.layers,function(key,o) {					
						...
						if (ct!==undefined && isvisible) o.timeline.time(ct);
	...
	***************************************/
	...
	createFrameOnTimeline : function(obj) {		
		...
		// ON START
		tweens.content.eventCallback("onStart",function(frame_index,ncobj,pw,_,tl,toanim,_nc){		
		...
		},[frame_index,_nc_tl_obj,_._pw,_,timeline,$to.anim,_nc ]);
		


		// ON UPDATE
		tweens.content.eventCallback("onUpdate",function(label,id,pw,_,tl,frame_index,toanim) {	
		...
		},[label,_._id,_._pw,_,timeline,frame_index,jQuery.extend(true,{},$to.anim)]);
		

		// ON COMPLETE
		tweens.content.eventCallback("onComplete",function(frame_index,frame_max,verylastframe,pw,_,tl) {										
			var data={};					
			data.layer = _nc;
		...
		},[frame_index,_.frames.length,verylastframe,_._pw,_,timeline]);
	...
	playAnimationFrame : function(obj) {
		...
		_R.createFrameOnTimeline({caption:obj.caption, timeline : _.triggeredtimeline, label:"triggered", frameindex : frameindex, opt:obj.opt, fromcurrentstate:fcs}).play();
